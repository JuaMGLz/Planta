...
function updateSensors() {
    ...
    document.getElementById('temperature').textContent = `${20 + Math.round(Math.random() * 8)}°C`;
    document.getElementById('light').textContent = `${50 + Math.round(Math.random() * 40)}%`;
    document.getElementById('ph').textContent = `${(6.0 + Math.random() * 1.5).toFixed(1)}`;
...
function updateTimestamp() {
    ...
    if (secondsPassed < 60) {
        document.getElementById('lastUpdate').textContent = `hace ${secondsPassed} seg`;
    } else {
        document.getElementById('lastUpdate').textContent = `hace ${Math.floor(secondsPassed / 60)} min`;
    }
}
