// === Rutas destino (auto-detecta GitHub Pages) ===
const IS_GH_PAGES = location.hostname === 'juamglz.github.io';
const BASE_PREFIX  = IS_GH_PAGES ? '/Planta' : ''; // en local: '', en GH Pages: '/Planta'
const DASHBOARD_PATH = `${BASE_PREFIX}/frontend/dashboard.html`;
...
async function handleAuth(event) {
  ...
  if (!login_input || !password) {
    return showStatus('⚠️ DATOS INCOMPLETOS: Correo/Usuario y Contraseña son requeridos.', false);
  }
  ...
  try {
    if (isLoginMode) {
      ...
      setTimeout(() => {
        location.href = new URL(DASHBOARD_PATH, location.origin).href;
      }, 700);
      ...
      const { error } = await supabase.auth.signUp({
        ...
        options: {
          ...
          emailRedirectTo: new URL(DASHBOARD_PATH, location.origin).href
        }
      });
      if (error) throw error;
...
async function loginWithGoogle() {
  try {
    ...
    const { error } = await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: {
        redirectTo: new URL(DASHBOARD_PATH, location.origin).href
      }
    });
    if (error) throw error;
  } catch (err) {
